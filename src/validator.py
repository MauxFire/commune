...
class ValidatorSession:
    ...
    def update_scores(
        ...
        print(f"Median of highest response times: {median_max_response_time}")
        print(f"Minimum response time: {min_response_time}")
    ...
    def update_weights(self, uids: torch.Tensor) -> None:
        ...
        print(f"Processed uids: {processed_uids}")

        is_weights_set, set_weights_msg = self.subtensor.set_weights(
            netuid=self.config.netuid,  # Subnet to set weights on.
            key=self.key,  # Wallet to sign set weights using hotkey.
    ...
    def try_store_scores(self):
        try:
            ...
            print(f"Error at storing scores {e}")
    ...
    def prepare_requests(self, uids: list[int]) -> list[dict]:
        ...
        if use_proof_of_weights:
            print("Preparing synapses for request to proof of weights")
        else:
            print("Preparing synapses for request to the default model")
    ...
    def run(self) -> NoReturn:
        ...
        while True:
            try:
                ...
                print("KeyboardInterrupt caught. Exiting validator.")
                clean_temp_files()
                sys.exit(0)
    ...
    def check_register(self):
        if self.wallet.hotkey.ss58_address not in self.metagraph.hotkeys:
            ...
            print(f"Running validator on uid: {subnet_uid}")
