...
def safe_login(api_key):
    ...
    """
    try:
        print("Attempting to log into WandB using provided API Key")
...
def safe_init(name, wallet, metagraph, config):
    ...
    if config.disable_wandb:
        ...
        return
    try:
        print("Attempting to initialize WandB")
        config_dict = {
        ...
        if torch.cuda.is_available():
            print("CUDA is available, attempting to get GPU specs...")
            try:
...
def safe_log(data):
    ...
    if not WANDB_ENABLED:
        print("Skipping log due to WandB logging disabled.")
        return

    try:
        print("Attempting to log data to WandB")
        wandb.log(data)
    except Exception as e:
